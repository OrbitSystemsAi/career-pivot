...
function controlButtonClass(selected: boolean) {
  return selected
    ? styles.button.borderedSelected
    : styles.button.bordered;
}

export default function ResumeDocument() {
  const [previewMode, setPreviewMode] =
    useState<PreviewMode>("preview");
  ...
  const optimizationSummary =
    parsedDocument?.optimizationSummary;
  ...
  function handleDownloadOriginal() {
    ...
    link.download =
      parsedDocument.originalFileName ??
  ...
  if (parsedDocument && !savedDocumentContent) {
    const canDownloadOriginal = Boolean(
      parsedDocument.originalFileDataUrl
    );
      ...
      <div className="h-full w-full overflow-auto">
        <div className={styles.document.toolbar}>
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <button
              onClick={handlePrint}
              className={styles.button.bordered}
            >
              Print
            </button>
        ...
        <div className={styles.document.canvas}>
          {previewMode === "preview" && (
            <div className="mx-auto h-full w-full max-w-[1000px]">
              ...
              <DocumentViewer document={parsedDocument} />
            </div>
          )}
            ...
            <div className="mx-auto w-full max-w-[816px]">
              <PanelCard title="Parser Debug">
                ...
                <ActionRow
                  label="Original Type"
                  value={
                    parsedDocument.originalFileType ??
                    "Unknown"
                  }
                />
                ...
                <ActionRow
                  ...
                  value={String(
                    structuredResume?.certifications.length ??
                      0
                  )}
                />
                ...
                <ActionRow
                  ...
                  value={String(
                    structuredResume?.unknownSections.length ??
                      0
                  )}
                />
                ...
                <ActionRow
                  ...
                  value={String(
                    structuredResume?.unclassifiedLines.length ??
                      0
                  )}
                />
              </PanelCard>
    ...
    savedDocumentContent ?? {
      ...
      highlights: [
        ...
        },
        {
          ...
          description:
            "Resume content adapts based on the selected target role.",
        },
      ],
    };
      ...
      <div className="min-h-full w-full rounded-2xl border border-slate-200 bg-white px-12 py-10 shadow-sm">
        <div className="border-b border-slate-200 pb-5">
          <div className="flex items-start justify-between">
            ...
            <div className="rounded-full bg-slate-100 px-4 py-2 text-xs text-slate-900">
              {activeVersion?.label}
            </div>
          </div>
        ...
        <section className="mt-6">
          ...
          <div className="mt-3 space-y-3 text-sm leading-6 text-slate-700">
            {documentContent.highlights.map(
              (highlight) => (
